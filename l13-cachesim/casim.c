...
unsigned long get_tag (unsigned long addr, unsigned long num_sets, unsigned short bsize) {
    ...
    unsigned long num_index = (unsigned long)log2(num_sets);

    unsigned long num_offset = (unsigned long)log2(bsize);

    unsigned long shift_amount = num_index + num_offset;

    unsigned long tag = addr >> shift_amount;
...
unsigned long get_index (unsigned long addr, unsigned long num_sets, unsigned short bsize) {
    ...
    unsigned long mask = (1 << num_index) - 1;

    unsigned long index = (addr >> num_offset) & mask;
...
int main(int argc, char **argv) {
    ...
    while (fscanf(tracefd, "0x%*x: %*c 0x%lx\n", &address) != EOF) {
        ...
        if (cache[index] == tag) {
            ...
            misses++;
            cache[index] = tag; // Update cache with the new tag
        }
